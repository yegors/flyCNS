...
export class FlyView {
  ...
  render(dt) {
    ...
    const m = nav?.active ? nav.motorSignals : this.app.frame?.playing ? this.app.ema : {};
    ...
    this.title.innerHTML = `<span style="color:var(--amber)">${mode}</span> · ${nav?.state === 'manual' ? 'brain signals' : nav?.busy ? 'street journey' : 'brain activity'}<br><span class="muted">${this.fly.reference ? 'Janelia / DeepMind · female anatomy, male CNS · illustrative motion' : 'Male illustration · 6 legs · 2 wings · halteres · sex combs'}</span>`;
